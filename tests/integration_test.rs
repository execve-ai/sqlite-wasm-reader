//! Integration tests for sqlite-wasm-reader
...
fn test_open_valid_database() {
    ...
    if let Err(e) = create_test_db(test_db) {
        eprintln!("Skipping test_open_valid_database: sqlite3 command not available: {}", e);
        return;
    }
...
fn test_read_users_table() {
    ...
    // Create a valid SQLite database
    if let Err(e) = create_test_db(test_db) {
        eprintln!("Skipping test_read_users_table: sqlite3 command not available: {}", e);
        return;
    }
    ...
    assert_eq!(alice.get("email"), Some(&Value::Text("alice@example.com".to_string())));
...
fn test_read_products_table() {
    ...
    // Create a valid SQLite database
    if let Err(e) = create_test_db(test_db) {
        eprintln!("Skipping test_read_products_table: sqlite3 command not available: {}", e);
        return;
    }
    ...
    assert_eq!(laptop.get("category"), Some(&Value::Text("Electronics".to_string())));
}
...
fn test_table_not_found() {
    ...
    // Create a valid SQLite database
    if let Err(e) = create_test_db(test_db) {
        eprintln!("Skipping test_table_not_found: sqlite3 command not available: {}", e);
        return;
    }
