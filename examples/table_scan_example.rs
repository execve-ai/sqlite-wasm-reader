...
use sqlite_wasm_reader::{Database, Error, Value, SelectQuery, init_default_logger, set_log_level, LogLevel};
use std::env;
        ...
        if let Some(first_row) = rows1.first() {
            if let Some(sample_value) = first_row.get(first_column) {
                let query2 = match sample_value {
                    Value::Text(s) => {
                        SelectQuery::parse(&format!("SELECT * FROM {} WHERE {} = '{}'", table_name, first_column, s))?
                    },
                    Value::Integer(i) => {
                        SelectQuery::parse(&format!("SELECT * FROM {} WHERE {} = {}", table_name, first_column, i))?
                    },
                    Value::Real(r) => {
                        SelectQuery::parse(&format!("SELECT * FROM {} WHERE {} = {}", table_name, first_column, r))?
                    },
                    _ => {
